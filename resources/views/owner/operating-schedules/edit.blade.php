...
@endphp
    ...
    @if ($errors->any())
        <div class="bg-red-100 text-red-600 p-4 rounded mb-4">{{ $errors->first() }}</div>
    ...
    <form action="{{ route('owner.operating-schedules.update',$field) }}" method="POST">
        ...
        @foreach ($field->operatingSchedules as $schedule)
            <div class="bg-white rounded-lg shadow p-6 mb-5">
                <h3 class="font-bold text-lg mb-4">{{ $days[$schedule->day_of_week] }}</h3>
        ...
        <div class="flex justify-end">
            <button class="bg-indigo-600 text-white px-6 py-2 rounded">Simpan Jadwal</button>
        </div>
    </form>
