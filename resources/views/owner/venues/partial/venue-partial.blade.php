<div class="p-4 rounded-md bg-slate-300 hover:bg-slate-400 transition">
    <h1 class="text-center font-semibold uppercase text-2xl">{{ $venue->name }}</h1>
    <p class="text-gray-500">Alamat: {{ $venue->address }}</p>
    <p>{{ $venue->description }}</p>
    <div class="flex flex-row justify-end mt-4 gap-4">
        <div>
            <a
                href="{{ route('venues.show', $venue) }}"
                class="inline-flex items-center py-2 px-4 bg-green-500 hover:bg-green-700 transition rounded-md font-semibold text-white"
            >
                View
            </a>
        </div>
        ...
        <div>
            <form action="{{ route('venues.destroy', $venue) }}" method="POST">
                ...
                <button
                    ...
                    class="inline-flex items-center py-2 px-4 bg-red-500 hover:bg-red-700 transition rounded-md font-semibold text-white"
                >
                    Hapus
                </button>
            </form>
